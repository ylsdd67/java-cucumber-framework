...
本框架使用 **Spring Boot + cucumber-spring** 作为 DI 容器。`TestContext` 标注了 `@Component` 和 `@ScenarioScope`，每个 Scenario 开始时：

1. Spring 容器创建一个新的 `TestContext` 实例（`@ScenarioScope` 保证场景隔离）
2. 所有 Step Definition 类的构造函数接收同一个 `TestContext`
3. Scenario 结束后，`TestContext` 被销毁
