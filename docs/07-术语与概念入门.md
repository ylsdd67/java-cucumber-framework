...
本框架使用 **Spring Boot + cucumber-spring** 作为 DI 容器。通过 `@CucumberContextConfiguration` 和 `@SpringBootTest` 桥接 Cucumber 生命周期与 Spring 容器，`TestContext` 标注了 `@Component` 和 `@ScenarioScope` 实现场景级别隔离。下面详细讲解它如何在本项目中运作。
...
public class Hooks {
    ...
    // Spring 看到构造器需要 TestContext，就自动注入一个 @ScenarioScope 实例
...
public class RestStepDefs {
    ...
    // Spring 注入的是 **同一个** TestContext 实例（@ScenarioScope）！
...
场景执行开始
  │
  ▼
Spring 容器看到 Hooks 和 RestStepDefs 都需要 TestContext
  │
  ├── 当前场景第一次请求 TestContext → 创建新实例（@ScenarioScope）
  │
  ├── Hooks(context)        ← 注入这个实例
  │
  └── RestStepDefs(context) ← 注入同一个实例！
  │
  ▼
...
```
Scenario 1: 查询用户
  ├── Spring 创建 TestContext #1（@ScenarioScope）
  ├── Hooks 使用 TestContext #1
  ├── RestStepDefs 使用 TestContext #1
  └── 场景结束 → TestContext #1 被销毁

Scenario 2: 创建帖子
  ├── Spring 创建 TestContext #2（@ScenarioScope） ← 全新的！
  ├── Hooks 使用 TestContext #2
  ├── RestStepDefs 使用 TestContext #2
  └── 场景结束 → TestContext #2 被销毁
```
...
| 特性     | 说明                                                     |
| -------- | -------------------------------------------------------- |
| 自动创建 | 不需要手动 `new TestContext()`，Spring 容器自动创建      |
| 共享实例 | 同一场景中，所有类拿到的是同一个 TestContext             |
| 场景隔离 | 每个场景有自己独立的 TestContext，互不干扰               |
| 自动清理 | 场景结束后 Spring 自动销毁 TestContext（@ScenarioScope） |
...
│ 断言库        │ AssertJ 3.27                      │
│ 依赖注入      │ Spring Boot + cucumber-spring        │
